...
def calculate_averages(csv_file_path):
    ...
    average_time_loss = total_time_loss / count if count > 0 else 0
    average_speed_factor = total_speed_factor / count if count > 0 else 0

    return average_speed, average_waiting_time, average_time_loss, average_speed_factor, count

if __name__ == "__main__":
    csv_file_path = '/home/haron/artery/scenarios/halmstad/extracted100.csv'
    avg_speed, avg_waiting_time, avg_time_loss, avg_speed_factor, num_ids = calculate_averages(csv_file_path)
    print(f"Average Speed: {avg_speed:.2f} km/h")
    print(f"Average Waiting Time: {avg_waiting_time:.2f} s")
    print(f"Average Time Loss: {avg_time_loss:.2f} s")
